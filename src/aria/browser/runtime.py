...
class PlaywrightBrowserRunner:
    ...
    def render(
        ...
        *,
        ...
        primary_domains = tuple(allowed_domains)
        dependency_domains = tuple(dependency_domains)
        network_domains = tuple(dict.fromkeys((*primary_domains, *dependency_domains)))
        policy = BrowserNetworkPolicy(
            allowed_domains=primary_domains,
            dependency_domains=dependency_domains,
            read_only_post_paths=read_only_post_paths,
                    ...
                    try:
                        ...
                        def route_request(route) -> None:
                            ...
                            if request.method.upper() == "POST":
                                try:
                                    ...
                                    request_body = b"\0" * (
                                        settings.BROWSER_MAX_REQUEST_BODY_BYTES + 1
                                    )
                        ...
                        if main_response is None:
                            raise BrowserNavigationError(
                                "Navigation produced no document response."
                            )
                        ...
                        self._close_resources(context, browser)
    ...
    @staticmethod
    def _close_resources(context, browser) -> None:
    ...
    def _attach_responses(policy: BrowserNetworkPolicy, responses) -> None:
        for response in responses:
            ...
            if (
                request.resource_type in {"document", "stylesheet", "script", "xhr", "fetch"}
