...
| Publish reviewed impact | latest approved/amended impact, current source confirmation, exact `PUBLISH` | idempotent impact publication + pipeline/outbox event + audit event |
...
Approval and publication remain separate on the impact detail page. The release action includes the
exact reviewed wording, controlled targets, and evidence in a durable event, while excluding private
business profiles. Optional HMAC-signed webhook delivery is configured through the
`ARIA_IMPACT_WEBHOOK_*` variables documented in the business-impact guide; with no URL, outbox rows
remain pending and the worker performs no outbound request.
