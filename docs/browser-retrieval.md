...
records audit and pipeline events, and schedules the first configured three-hour check. It cannot bypass a failed
gate.
...
guarded command and now polls at the governed three-hour cadence; reseeding preserves that promoted state. Connector version 2
